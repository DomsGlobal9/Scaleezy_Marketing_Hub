...
export const Route = createFileRoute("/social/linkedin/callback")({
...
function LinkedInCallbackPage() {
  ...
  useEffect(() => {
    ...
    // Handle LinkedIn OAuth errors (user denied, etc.)
    if (error) {
      const msg = errorDescription || "LinkedIn authorization was not granted.";
      setStatus("error");
      setErrorMessage(msg);
      toast.error(msg);
    ...
    apiFetch("/api/marketing/social-accounts/oauth_callback/", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          const msg = data.message || data.error?.message || "Failed to connect LinkedIn.";
          setStatus("error");
          setErrorMessage(msg);
          toast.error(msg);
          setTimeout(() => navigate({ to: "/accounts" }), 3000);
      ...
      .catch((err) => {
        ...
        setStatus("error");
        setErrorMessage("Network error while connecting LinkedIn.");
        toast.error("Network error during LinkedIn connection.");
        setTimeout(() => navigate({ to: "/accounts" }), 3000);
      });
  }, [navigate]);
