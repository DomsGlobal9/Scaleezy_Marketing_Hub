...
function cleanFrom(raw: unknown): Guardrails {
  ...
  const list = (key: string) =>
    Array.isArray(source[key])
      ? (source[key] as unknown[]).map(String).filter(Boolean)
      : [];
  return {
    ...
    language_rule:
      source["language_rule"] === "english_only" ||
      source["language_rule"] === "hinglish_allowed"
...
export function HardRulesPanel({ brandId }: { brandId: string }) {
  ...
  useEffect(() => {
    ...
    api<{ guardrails?: unknown }>(`/api/marketing/brands/${brandId}/`)
      ...
      .catch((e) => {
        if (!cancelled)
          setError(e instanceof Error ? e.message : "Could not load hard rules.");
      });
    return () => {
      cancelled = true;
    };
