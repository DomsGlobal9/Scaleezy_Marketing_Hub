...
function LayoutChoice({
  ...
  return (
    <button
      ...
      onClick={onChoose}
      className={cn(
        "min-w-40 flex-1 rounded-xl border p-3 text-left transition-colors",
        active
    ...
    >
      <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
        ...
        {option?.display_name ?? "Scaleezy decides"}
      </span>
      <span className="mt-1 block text-xs text-muted-foreground">
        {option?.description ?? "Use the brand preference and campaign context."}
      </span>
    </button>
  );
}
...
export function CreativeCommand({
  ...
  const catalogue = useLayoutCatalogue();
  ...
  return (
    <section className="mt-8 rounded-2xl border border-border bg-secondary/20 p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-muted-foreground">
            Choose any number of real references. Tell Scaleezy what to use, what to avoid, and
            which creative details matter for this generation.
          </p>
        </div>
      ...
      <div className="mt-5">
        <Label className="text-xs uppercase tracking-wide">Composition</Label>
        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
          <LayoutChoice option={null} active={!layout} onChoose={() => onLayoutChange("")} />
          {catalogue.layouts.map((option) => (
            <LayoutChoice
              key={option.key}
              option={option}
        ...
        <div className="flex flex-wrap gap-1.5" aria-label="Reference source">
          {(
            ...
            <button
              ...
              // Transition: switching re-renders the whole card grid; sliced,
              // the click paints inside the 200ms INP budget.
      ...
      {loading ? (
        ...
        <div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
          No references match this view. Add inspirations in Brand Master or ask the Scaleezy team
          to publish more to the shared library.
        </div>
      ) : (
